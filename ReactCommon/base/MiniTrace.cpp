...
double between_bridge_create_and_run_bundle_js = 0;
...
static volatile bool has_inited;
...
void mtr_init_from_stream(void *stream) {
	...
	// __android_log_print(ANDROID_LOG_ERROR, "ReactNative::minitrace", "mtr_init");
}

void free_file_name_if_needed() {
  if (file_name != NULL) {
    free(file_name);
    file_name = NULL;
  }
...
void mtr_init(const char *json_file) {
  ...
  has_inited = true;
  file_name = (char*)malloc(strlen(json_file) + 1);
  strcpy(file_name, json_file);
...
void mtr_shutdown() {
  ...
  has_inited = false;
	...
	fwrite("\n]}\n", 1, 4, f);
	fclose(f);
	// __android_log_print(ANDROID_LOG_ERROR, "ReactNative::minitrace", "mtr_shutdown %s", file_name);
...
void mtr_flush_with_state(int is_last) {
	...
	event_count = 0;
    // __android_log_print(ANDROID_LOG_ERROR, "ReactNative::minitrace", "mtr_flush_begin");
	...
	for (i = 0; i < event_count_copy; i++) {
		...
		// __android_log_print(ANDROID_LOG_ERROR, "ReactNative::minitrace", "mtr_flush %s", linebuf);
...
void internal_mtr_raw_event(const char *category, const char *name, char ph, void *id) {
  // __android_log_print(ANDROID_LOG_ERROR, "ReactNative::minitrace", "internal_mtr_raw_event");
#ifndef MTR_ENABLED
	return;
#endif
	pthread_mutex_lock(&mutex);
	...
	ev->cat = (char *)malloc(category_len + 1);
	strcpy(ev->cat, category);

	const size_t name_len = strlen(name);
	ev->name = (char *)malloc(name_len + 1);
	strcpy(ev->name, name);
